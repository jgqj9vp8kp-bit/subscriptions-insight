...
function row(r: Record<string, unknown>): SupportRequestRow {
  return {
    ...
    secondary_categories: Array.isArray(r.secondary_categories)
      ? (r.secondary_categories as unknown[]).map((value) => s(value)).filter((value) => value.length > 0)
      : [],
    classification_source: s(r.classification_source) || "rule",
    classification_model: s(r.classification_model) || null,
  ...
  if (rowsScanned > 0) {
    ...
    await input.clickhouse.command({ query: `
      ...
      SELECT
        ...
        s.sender, s.matched_contact_name, s.language, s.category, s.subcategory, s.secondary_categories,
