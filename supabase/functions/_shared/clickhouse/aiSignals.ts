// Deterministic AI-analytics signal engine (ai-signals-v1).
//
...
export const AI_SIGNALS_ENGINE_VERSION = "ai-signals-v1";
...
export interface AiEngineInput {
  ...
  /** Billing period in days, default 30 (retention observability window). */
  periodDays?: number;
...
function buildCohortPools(
  ...
  const pools: CohortPools = {
    ...
    refundUserByPath: new Map(),
    refundUserGlobal: EMPTY_POOL,
  ...
  for (const row of rows) {
    ...
    if (row.trial_users > 0) {
      bump(pools.refundUserByPath, path, row.refund_users, row.trial_users);
      pools.refundUserGlobal = addPool(pools.refundUserGlobal, row.refund_users, row.trial_users);
      if (typeof row.upsell_revenue === "number") {
...
interface PathTrend {
  ...
  cpaImproving: boolean | null;
  convDeteriorating: boolean | null;
  convImproving: boolean | null;
  known: boolean;
  signals: AiSignal[];
}
...
function buildPathTrends(
  ...
  for (const [path, list] of byPath) {
    ...
    const trend: PathTrend = {
      cpaDeteriorating: null, cpaImproving: null, convDeteriorating: null, convImproving: null,
      known: false, signals: [],
    };
    ...
    if (spendRows.length >= TREND_WINDOW_COHORTS * 2) {
      ...
      trend.cpaImproving = better === null ? null : better;
    ...
    if (matureRows.length >= TREND_WINDOW_COHORTS * 2) {
      ...
      trend.convImproving = better === null ? null : better;
      trend.convDeteriorating = better === null ? null : !better;
    }
...
function signalsForAnalysis(a: RowAnalysis, thresholds: AiThresholds, confidence: { bucket: AiConfidence; score: number }): AiSignal[] {
  ...
  if (a.upsellPerTrial !== null) {
    if (a.upsellPerTrial >= thresholds.upsellRevenuePerTrialTarget) {
      push("HIGH_UPSELL_REVENUE", "upsell", "good", "low", "cpa", `Upsell revenue ${renderMoney(a.upsellPerTrial)} per trial.`);
    } else if (a.upsellPerTrial < thresholds.upsellRevenuePerTrialTarget / 2) {
      push("LOW_UPSELL_REVENUE", "upsell", "bad", "info", "cpa", `Upsell revenue ${renderMoney(a.upsellPerTrial)} per trial vs ${renderMoney(thresholds.upsellRevenuePerTrialTarget)} target.`);
    }
  }
...
function analysisConfidence(a: RowAnalysis, thresholds: AiThresholds): { bucket: AiConfidence; score: number } {
  ...
  const score = round2(Math.min(1, sf * maturityFactor * spendFactor * benchmarkFactor));
...
const BECAUSE_ORDER = ["cpa", "trial_to_paid", "pass_rate", "ltv_cpa", "payback", "retention_c2", "refund_rate", "roas"];
...
function buildOpportunities(
  ...
  const scored = recommendations
    ...
    .map((rec) => {
      ...
      const maturityFactor = a?.surface === "campaign" ? 0.8 : a?.conv !== null ? 1 : 0.6;
      const spendFactor = a?.spendInfo.usable ? a.spendInfo.factor : 0.7;
      return {
  ...
  for (const opp of scored) {
    const path = opp.recommendation.scope.kind === "campaign"
      ? (analyses.get(scopeKey(opp.recommendation.scope))?.scope.kind === "campaign" ? opp.recommendation.scope.campaignId : "")
      : opp.recommendation.scope.campaignPath;
    const used = perPath.get(path) ?? 0;
    if (used >= OPPORTUNITY_PER_PATH_CAP) continue;
