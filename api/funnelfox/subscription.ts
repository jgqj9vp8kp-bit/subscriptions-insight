...
import { handleFunnelFoxSubscriptionDetails } from "./subscriptionsCore";
...
function getSubscriptionId(req: ApiRequest): string {
  ...
  const url = new URL(req.url ?? "/api/funnelfox/subscription", `https://${host}`);
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  const result = await handleFunnelFoxSubscriptionDetails({
