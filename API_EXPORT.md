...
| `net_revenue` | Чистая выручка группы (gross успешных продаж − рефанды/чарджбэки), USD — по курсу, см. раздел «Валюта» |
...
### Валюта: `net_revenue` исправлен (важно при сравнении с выгрузками до 30.07.2026)

`net_revenue` всегда в USD. До перехода на ClickHouse выгрузка брала суммы из JSON-поля
`normalized_payload.net_amount_usd`, которое, несмотря на название, хранит сумму **в валюте платежа**
и не содержит признака конвертации (`fx_status` там не заполняется никогда). Из-за этого
23,7% транзакций склада (8 944 из 37 757 — MXN, COP, ARS, EUR, ZAR, PHP, CAD) попадали в
`net_revenue` как доллары: COP 105 000 считались как $105 000 вместо $26,25, MXN 519 — как $519
вместо $28,03. Выручка кампаний с трафиком из Латинской Америки была завышена в разы.

Теперь суммы читаются из колонок склада ClickHouse, куда они попадают уже сконвертированными
(`convertAmountToUsd`, курсы из статического конфига). Поэтому `net_revenue` — и производный от
него `roas`, когда он снова начнёт выгружаться, — для таких кампаний стали **меньше и корректны**.
Счётчики пользователей (`trial_users`, `upsell_users`, `first_sub_users`, `refund_users`) и все
конверсии не изменились: они не зависят от сумм.

## 4a. Гео-разбивка: `?breakdown=country`

Параметр `breakdown=country` (синоним `geo`) переключает форму строк на **посуточную разбивку по странам**. Конверт тот же (`GET`, `Bearer`, `{ "data": [...] }`), остальные параметры (`date_from`, `date_to`, `campaign_path`, `media_buyer`, `campaign_id`) работают как раньше. Без параметра ответ полностью совпадает с прежним контрактом.
...
| Свойство | Гарантия |
|---|---|
| Гранулярность | Одна строка = кампания × день × страна; `date_from == date_to` всегда, даже при запросе диапазона |
| Датировка | По дню **первого успешного trial** пользователя — тот же принцип, что у обычных строк |
| `country` | ISO 3166-1 alpha-2, заглавными (`US`, `DE`, `BR`) |
| Гео неизвестно | Явная строка с кодом **`ZZ`** — такие пользователи не растворяются в других странах и не выпадают молча |
| Инвариант сверки | Сумма по всем странам за кампанию+день **равна** тоталу обычной выгрузки за эту кампанию+день (оба режима строят строки из одного и того же множества пользователей) |
| `campaign_id` | Всегда строка (JSON string) — 17-значные Meta ID не передаются числом |
| `campaign_name` | Последнее известное имя из FB-склада; `null`, если кампания там не встречалась |
| CR | Не выгружаются: `upsell_cr` / `trial_to_first_sub_cr` в гео-строках отсутствуют, платформа-потребитель считает их сама |
| `media_buyer` / `utm_source` | Те же значения, что в остальной аналитике (маппинг utm_source → байер) |
| `failed_payment_users` | Уникальные пользователи кампании+дня+страны хотя бы с одной неуспешной попыткой оплаты |

В `meta` ответа добавляется поле `breakdown: "country"` (в обычном режиме — `null`).

Пример запроса за один день:

```
GET /functions/v1/export-campaign-performance?breakdown=country&date_from=2026-08-13&date_to=2026-08-13
```
