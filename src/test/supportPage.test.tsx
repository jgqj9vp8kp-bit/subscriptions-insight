  ...
  return {
    ...
    summary: {
      ...
      kpis: {
        ...
        answeredContacts: pageRows.filter((row) => row.answered).length,
        unansweredContacts: pageRows.filter((row) => !row.answered).length,
        answerableContacts: pageRows.length,
        answerRatePct: 50,
        medianFirstResponseMinutes: 120,
      },
      byDay: [{ date: "2026-06-30", requests: pageRows.length, answered: pageRows.filter((row) => row.answered).length, answerable: pageRows.length }],
